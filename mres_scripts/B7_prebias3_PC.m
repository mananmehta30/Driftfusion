...
Vappt_ss = dfana.calcVapp(SteadyState_CV);
...
for Vi = 1:length(V_fr_arr)
    % Which voltage
    Vpre = V_fr_arr(Vi);
    disp(['Vpre = ', num2str(Vpre), ' V'])
    index = find(Vappt_ss(501:1301) <= Vpre);  % Not general solution to how to find the correct time point
    index = index(1)+500;
    
    sol_ic(Vi) = extract_IC(SteadyState_CV, SteadyState_CV.t(index));
    sol_ic(Vi).par.mobseti = 0;
...
%     tpoints = 11;
%     logtime = 0;
%     Vpre_check(Vi) = sol_ic(Vi).par.Vapp;
%     disp('Sweep to Vapp = 0 V for consistent starting point')
%     sol_0V(Vi) = VappFunction(sol_ic(Vi), 'sweep', [Vpre_check(Vi), 0, tmax], tmax, tpoints, logtime);
%     disp('Complete')
    
    % Run CV
    % sol_CV = doCV(sol_ini, light_intensity, V0, Vmax, Vmin, scan_rate, cycles, tpoints)
    sol_CV_fr(Vi) = doCV(sol_ic(Vi), 0, 0, 3, -3, 0.01, 1, 2001);
...
%         sol_CV_fr(Vi) = doCV(sol_eq.ion, 0, 0, 5, -5, 0.01, 1, 2001);
