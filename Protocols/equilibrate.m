...
%     
    ...
    par.Kcat_l = min(par.sn_l, par.sp_l)/par.sc_l;
    par.Kcat_r = min(par.sn_r, par.sp_r)/par.sc_r;

    if isnan(par.Kcat_l) || isinf(par.Kcat_l)
        par.Kcat_l = 0;
    end
    
    if isnan(par.Kcat_r) || isinf(par.Kcat_r)
        par.Kcat_r = 0;
    end
