...
% S = Source terms
    function [C,F,S] = dfpde(x,t,u,dudx)
        ...
        if N_ionic_species == 1 || N_ionic_species == 2  % Condition for cation and anion terms
            ...
            % change cation flux to workaround time integration failure at DOScat
                    if c > 0.99*DOScat(i)
                        c_dash = 0.99*DOScat(i);
                    else
                        c_dash = c;
                    end

            F_cation = mucat(i)*(c*dVdx + kB*T*(dcdx + (c_dash*(dcdx/(DOScat(i)-c_dash)))));
        ...
        switch par.BC
            ...
            case 3
                ...
                if N_ionic_species == 1 || N_ionic_species == 2
                    ...
                    % coefficients for CATIONS
                    Pl_cation = mobseti*par.Kcat_l*-sc_l*(ul(4) - Ncat(1));
                    Ql_cation = 1;

                    Pr_cation = mobseti*par.Kcat_r*sc_r*(ur(4) - Ncat(end));
                    Qr_cation = 1;
